...
- DEV-119 siege result → logical-force merge: IMPLEMENTED. ArmyState now owns authoritative per-TroopType composition while legacy baseTroops stays synchronized to the exact total. Existing total-only constructors migrate troops to INFANTRY for compatibility. Core DEV-119 merge validates the battle-start DEV-117 Mapping against the current army, projects survivors per troop type, rejects stale/invalid results before mutation, and atomically replaces composition. Snapshot schema v21 persists exact troop composition; legacy v1-v20 army rows restore baseTroops as INFANTRY-only. Paper live mapped battles retain per-side mappings and expose surviving AI counts by troop type through LiveMappedBattleResult without assuming the provisional 100:1 ratio. Windows quick-deploy verification pending.
...
21. DEV-119 siege result → logical-force merge: IMPLEMENTED, awaiting Windows quick-deploy verification. Snapshot schema is now v21 with v1-v20 legacy army migration.
22. After DEV-119 GREEN/live contract verification, next execution target is DEV-120 siege snapshot/recovery.
