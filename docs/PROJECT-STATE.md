...
- WORLD-HARDEN-001 strategic-mainland ambient mob suppression: IMPLEMENTED. `danta_main` blocks all non-CUSTOM CreatureSpawnEvent reasons while `danta_wild` and other worlds keep normal spawning. Plugin-managed CUSTOM summons (including siege/combat AI) remain allowed. On world-runtime initialization, existing hostile/slime entities in the strategic world are removed once. `/danta world status` reports the policy. Windows quick-deploy/live verification pending.
...
20. WORLD-HARDEN-001 strategic-mainland ambient mob suppression: IMPLEMENTED, awaiting Windows quick-deploy and live verification.
