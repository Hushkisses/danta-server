...
Updated: 2026-09-16
Checkpoint: DEV-102 COMPLETE; DEV-103 IMPLEMENTED awaiting Windows verification
...
- DEV-103 current hegemony score: IMPLEMENTED. Added a recomputable current-state score service rather than a second persisted mutable counter. It derives score from authoritative current strategic-point ownership through an injected policy, so ownership loss/gain changes the result immediately. Exact strategic-point weights and the remaining current-state sources (vassals, NPC subjugation, etc.) are deliberately deferred to DEV-104 instead of inventing balance values. Windows quick-deploy automated tests/build pending.
...
4. DEV-103 current hegemony score is IMPLEMENTED and awaits Windows quick-deploy verification. Do not begin DEV-104 until verification succeeds; final scoring sources/weights belong to DEV-104.
5. Current clean live baseline after DEV-101 verification: active wars 0, runtime 25:16:29, speed x1.0, season CONFLICT.
6. Observed during dev runtime restoration: advancing runtime from the 4h test range back to ~25h16m caused EconomyTick to catch up 42 ticks at once. Treat this as an observation for future runtime manipulation/snapshot/economy scheduler review, not as a DEV-101 defect.
