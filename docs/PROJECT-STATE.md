...
## Verified tickets
- DEV-117 logical-force ↔ live-AI mapping: COMPLETE. Windows quick-deploy automated tests/build passed. Core mapping is configurable with provisional development default 100 logical troops per live AI, uses per-troop-type representative counts, projects survivors by representative survival ratio, and supports asymmetric Paper live formations. Final ratio remains intentionally unresolved pending later server-performance/scale testing.
...
- DEV-120 siege snapshot/recovery: GREEN. Snapshot schema v22 now persists logical SiegeInstance lifecycle/result, wall-clock SiegeReservation timestamps, staged Paper siege progress, DEV-118 participant side/elimination/original-game-mode state, and accumulated commander morale penalties. v1-v21 decode with empty siege state. Core siege/reservation services have explicit restore paths; DantaSiegeRuntime exports/imports Paper state, marks unfinished restored stages as `[재개 대기]`, reapplies eliminated-player restrictions on login, and triggers important snapshot flushes on siege mutations. Live mobs/projectiles are intentionally not serialized; a restart resumes from the logical stage boundary. Windows quick-deploy automated tests/build passed on 2026-09-20 after correcting two invalid backward-compatibility test fixtures; live Paper restart recovery verification remains pending.
- DEV-119 siege result → logical-force merge: GREEN. Windows quick-deploy automated tests/build passed after correcting the legacy-v20 fixture to match the actual historical empty-vassal encoding. ArmyState owns authoritative per-TroopType composition while legacy baseTroops stays synchronized to the exact total. Existing total-only constructors migrate troops to INFANTRY for compatibility. Core DEV-119 merge validates the battle-start DEV-117 Mapping against the current army, projects survivors per troop type, rejects stale/invalid results before mutation, and atomically replaces composition. Snapshot schema v21 persists exact troop composition; legacy v1-v20 army rows restore baseTroops as INFANTRY-only. Paper live mapped battles retain per-side mappings and expose surviving AI counts by troop type through LiveMappedBattleResult without assuming the provisional 100:1 ratio.
- WORLD-HARDEN-001 strategic-mainland ambient mob suppression: GREEN. Windows quick-deploy automated tests/build passed. `danta_main` blocks all non-CUSTOM CreatureSpawnEvent reasons while `danta_wild` and other worlds keep normal spawning. Plugin-managed CUSTOM summons (including siege/combat AI) remain allowed. On world-runtime initialization, existing hostile/slime entities in the strategic world are removed once. `/danta world status` reports the policy. Live verification of ambient suppression + CUSTOM combat-AI spawning remains pending.
- DEV-118 player commander incapacitation / no-reentry: COMPLETE. Windows quick-deploy and live Paper verification passed. Registered siege commanders are intercepted on lethal damage, prevented from normal death/drop, switched to spectator, blocked from rejoining that siege until it ends, and restored when the siege is cleared/completed. Provisional configurable commander-loss penalty records commander bonus disabled plus morale -20 per casualty/side; numeric morale-stage conversion remains intentionally deferred. Development participation uses explicit `<attack|defend>` until authoritative player→nation membership exists. Participant persistence remains deferred to DEV-120.
- DEV-112A generated physical canyon fortress: IMPLEMENTED (visual revision 2). Added deterministic Paper-side builder and OP-only `/danta map build-fortress`. It creates a ~96x96 curtain-wall fortress inside the reserved 128x128 battlefield, four towers, west-facing outer gate, inner keep wall/gate, central keep, core beacon/dais, assault road and basic lighting, aligned to DEV-112 objective anchors. This command intentionally rewrites the authored test volume around canyon_fort and is for explicit admin setup only, not normal runtime regeneration. First live visual pass confirmed successful physical generation but showed excessive darkness/empty courtyard. Revision 2 removes visible torch lighting in favor of invisible LIGHT blocks and adds courtyard gardens, supply/market stalls, blue military banners, and a taller command tower for stronger visual hierarchy. Live Minecraft visual/coordinate verification of revision 2 pending before COMPLETE. Later polished assets may replace/export this generator as NBT/schematic content.
...
18. DEV-117 logical-force ↔ live-AI mapping: COMPLETE after Windows quick-deploy success. Temporary development ratio is 100:1 and configurable; final ratio remains deferred to later performance testing.
19. DEV-118 player commander incapacitation / no-reentry: COMPLETE after Windows quick-deploy and live lethal-damage/rejoin-rejection/clear verification.
20. WORLD-HARDEN-001 strategic-mainland ambient mob suppression: GREEN after Windows quick-deploy success; live suppression/CUSTOM-spawn verification remains deferred.
21. DEV-119 siege result → logical-force merge: GREEN after Windows quick-deploy automated tests/build. Snapshot schema is v21 with v1-v20 legacy army migration.
22. DEV-120 siege snapshot/recovery: GREEN after Windows quick-deploy automated tests/build success on 2026-09-20. Snapshot schema is now v22; live Paper entities are intentionally excluded from persistence. Awaiting live Paper restart recovery verification before COMPLETE.
23. After DEV-120 verification, Phase 11 core siege persistence/mapping chain is complete enough to proceed to the next planned phase or integration hardening.
