...
Updated: 2026-09-16
Checkpoint: DEV-106 COMPLETE; DEV-107 IMPLEMENTED awaiting Windows verification
...
- DEV-107 chronicle event log: IMPLEMENTED. Added core chronological ChronicleService with server-runtime timestamps and stable event categories, Snapshot schema v20 persistence with v1-v19 backward compatibility, Paper binding/restore, Korean `/danta chronicle` showing the most recent 20 entries, automatic recording for authoritative StrategicPointOwnershipChangedEvent, and administrator season-end recording. Other major event sources (war/vassal/research/facility) currently lack a common domain-event boundary and are not duplicated or invasive-wired in DEV-107; they can publish/record through this foundation as their services are evolved. Windows quick-deploy, restart persistence, and live command verification pending.
...
9. DEV-107 chronicle event log is IMPLEMENTED and awaits Windows quick-deploy plus live `/danta chronicle` and restart-persistence verification.
...
11. Current live baseline restored after admin-end test: runtime 25:18:52, speed x1.0, season CONFLICT, runtime running.\n10. Previous clean live baseline after DEV-101 verification: active wars 0, runtime 25:16:29, speed x1.0, season CONFLICT.
13. Observed during dev runtime restoration: advancing runtime from the 4h test range back to ~25h16m caused EconomyTick to catch up 42 ticks at once. Treat this as an observation for future runtime manipulation/snapshot/economy scheduler review, not as a DEV-101 defect.
