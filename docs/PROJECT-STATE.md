...
- DEV-112A generated physical canyon fortress: IMPLEMENTED. Added deterministic Paper-side builder and OP-only `/danta map build-fortress`. It creates a ~96x96 curtain-wall fortress inside the reserved 128x128 battlefield, four towers, west-facing outer gate, inner keep wall/gate, central keep, core beacon/dais, assault road and basic lighting, aligned to DEV-112 objective anchors. This command intentionally rewrites the authored test volume around canyon_fort and is for explicit admin setup only, not normal runtime regeneration. Live Minecraft visual/coordinate verification pending before COMPLETE. Later polished assets may replace/export this generator as NBT/schematic content.
