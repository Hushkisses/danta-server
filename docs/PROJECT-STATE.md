...
- DEV-118 player commander incapacitation / no-reentry: IMPLEMENTED. Approved design replaces timed re-entry with one-life-per-siege commander elimination. Lethal damage is intercepted for registered siege commanders, death/drop is prevented, the player is moved to spectator mode and cannot rejoin that siege until it ends. Provisional configurable commander-loss penalty records commander bonus disabled plus morale -20 per casualty/side; numeric morale-stage conversion is intentionally deferred because current core morale is stage-based and no final -20→stage threshold is defined. Development participation uses explicit `<attack|defend>` because authoritative player→nation membership does not yet exist. Participant state remains in-memory until DEV-120 snapshot/recovery. Windows quick-deploy/live verification pending.
...
19. DEV-118 player commander incapacitation / no-reentry: IMPLEMENTED, awaiting Windows quick-deploy and live lethal-damage/clear verification.
