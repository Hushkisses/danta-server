...
Updated: 2026-09-16
Checkpoint: DEV-105 COMPLETE; DEV-106 IMPLEMENTED awaiting Windows verification
...
- DEV-106 ranking UI/season result: IMPLEMENTED. Added core NationRankingService projecting each nation's accumulated fame + current hegemony into a deterministic ranking, and Korean `/danta ranking` output that labels live ranking before FINISHED and final result after FINISHED. Tie handling is deterministic (total, hegemony, fame, nation ID) but no extra gameplay tie-break rule was invented. Paper wiring currently uses a zero-valued provisional HegemonyScorePolicy because final numeric score weights remain unresolved; DEV-104/105 scoring mechanics stay reusable once weights are configured. Windows quick-deploy and live command verification pending.
...
8. DEV-106 ranking UI/season result is IMPLEMENTED and awaits Windows quick-deploy plus `/danta ranking` live verification.
...
10. Current live baseline restored after admin-end test: runtime 25:18:52, speed x1.0, season CONFLICT, runtime running.\n10. Previous clean live baseline after DEV-101 verification: active wars 0, runtime 25:16:29, speed x1.0, season CONFLICT.
12. Observed during dev runtime restoration: advancing runtime from the 4h test range back to ~25h16m caused EconomyTick to catch up 42 ticks at once. Treat this as an observation for future runtime manipulation/snapshot/economy scheduler review, not as a DEV-101 defect.
