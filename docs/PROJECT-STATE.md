...
Updated: 2026-09-16
Checkpoint: execution-plan DEV-104 COMPLETE; DEV-105 IMPLEMENTED awaiting Windows verification
...
- Execution-plan DEV-105 same-kind major-point diminishing returns: IMPLEMENTED. Design v0.3 rule 100%→75%→50% is applied to current hegemony scoring for same-kind MAJOR points. Because current StrategicPointType has only broad MAJOR, the scoring policy exposes a stable majorPointKind hook for later fortress/market/etc. content without changing aggregation. The document does not specify a fourth+ step, so the third 50% is retained as a floor rather than inventing further decay. Windows quick-deploy verification pending.
...
7. Execution-plan DEV-105 diminishing returns is IMPLEMENTED and awaits Windows quick-deploy verification.
